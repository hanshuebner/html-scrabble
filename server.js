...
Game.prototype.sendInvitation = async function (player, subject) {
  try {
    ...
    if (smtp !== undefined) {
      const mailResult = await smtp.sendMail({
  ...
  return [
    [],
    {
      ...
      placements: previousMove.placements.map(function (placement) {
        return { x: placement[1].x, y: placement[1].y };
      }),
...
Game.prototype.pass = function (player) {
  ...
  return [[], { type: "pass", score: 0, player: player.index }];
};
...
Game.prototype.returnPlayerLetters = function (player, letters) {
  ...
  if (lettersToReturn.contents.length) {
    throw (
      "could not find letters " +
      lettersToReturn.contents +
      " to return on player " +
      player +
      "'s rack"
    );
...
Game.prototype.swapTiles = function (player, letters) {
  ...
  if (game.letterBag.remainingTileCount() < 7) {
    throw (
      "cannot swap, letterbag contains only " +
  ...
  return [
    newTiles,
    { type: "swap", score: 0, count: letters.length, player: player.index },
  ];
};
...
Game.prototype.finishTurn = function (player, newTiles, turn) {
  ...
  if (game.passes == game.players.length * 2) {
    ...
    _.every(player.rack.squares, function (square) {
      return !square.tile;
    })
  ) {
...
Game.prototype.createFollowonGame = async function (startPlayer) {
  if (this.nextGameKey) {
    throw (
      "followon game already created: old " +
      this.key +
      " new " +
      this.nextGameKey
    );
...
var gameListAuth = basicAuth(function (username, password) {
  if (config.gameListLogin) {
    return (
      username == config.gameListLogin.username &&
...
app.get("/game/:gameKey/:playerKey", (req, res) =>
  getGameAndSetPlayerKey(req, res)
),
        ...
        if (!game) {
          ...
          game.players.map(function (player_) {
            if (player_.key == data.playerKey) {
              ...
              if (
                _.find(game.connections, function (connection) {
                  return connection.player == player_;
                })
              ) {
