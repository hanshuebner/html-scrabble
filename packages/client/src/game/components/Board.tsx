...
export const Board = () => {
  ...
  for (let y = 0; y < 15; y++) {
    for (let x = 0; x < 15; x++) {
      ...
      const isSelected =
        selectedSquare !== null && !selectedSquare.fromRack && selectedSquare.x === x && selectedSquare.y === y

      const displayTile = pending ? { letter: pending.letter, score: pending.score } : sq.tile
  ...
  return (
    <div className="@container grid grid-cols-[repeat(15,1fr)] gap-0 w-[calc(100vw-1rem)] max-w-[min(43.75rem,100%)] aspect-square bg-[#54534A] p-[2px] rounded shadow-lg desktop:w-[min(43.75rem,calc((100dvh-3rem)*15/16))] select-none">
      {squares}
    </div>
  )
}
