...
export function ChatPanel() {
  ...
  return (
    <div className="bg-[#F7F7E3] border border-[#DCDCC6] rounded-md p-3">
      <h3 className="font-bold text-sm text-[#474633] mb-2">Chat</h3>
      <div ref={scrollRef} className="h-24 overflow-auto text-xs space-y-1 mb-2">
        {chatMessages.length === 0 && (
          <div className="text-[#AAA38E] italic">No messages</div>
        )}
      ...
      <div className="flex gap-1">
        ...
        <button
          onClick={handleSend}
          className="text-xs px-2 py-1 bg-[#474633] text-white rounded hover:bg-[#626258]"
        >
          Send
        </button>
      </div>
