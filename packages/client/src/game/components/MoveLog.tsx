...
export const MoveLog = () => {
  ...
  return (
    <div className="bg-[#F7F7E3] border border-[#DCDCC6] rounded-md p-3">
      <h3 className="font-bold text-sm text-[#474633] mb-2">Move Log</h3>
      <div ref={scrollRef} className="h-40 overflow-auto text-xs space-y-1">
        {turns.length === 0 && (
          <div className="text-[#AAA38E] italic">No moves yet</div>
        )}
        {turns.map((turn, i) => {
          ...
          return (
            <div key={i} className="border-b border-[#DCDCC6] pb-1">
              <div className="flex justify-between">
                ...
                {turn.type === 'move' && (
                  <span className="text-green-700 font-bold">+{turn.score}</span>
                )}
                {turn.type === 'pass' && (
                  <span className="text-[#AAA38E]">passed</span>
                )}
                {turn.type === 'swap' && (
                  <span className="text-blue-600">swapped {turn.count}</span>
                )}
                {turn.type === 'challenge' && (
                  <span className="text-red-600">{turn.score}</span>
                )}
                {turn.type === 'takeBack' && (
                  <span className="text-orange-600">{turn.score}</span>
                )}
              </div>
