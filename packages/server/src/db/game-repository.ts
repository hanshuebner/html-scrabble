...
export const insertGameWithPlayers = async (gameData: GameInsertData, players: PlayerInsertData[]): Promise<string> => {
...
export const findGameByKey = async (
  ...
  const [gameRow] = await db.select().from(games).where(eq(games.key, key)).limit(1)
  ...
  const turnRows = await db.select().from(turns).where(eq(turns.gameId, gameRow.id)).orderBy(turns.turnNumber)
