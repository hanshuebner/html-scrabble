...
function calculateMove(squares) {
    ...
    if (!squares[7][7].tile) {
        return {
            error: "start field must be used"
        };
    }
    ...
    for (var x = topLeftX + 1; x < 15; x++) {
        if (!squares[x][topLeftY].tile) {
            break;
        } else if (!squares[x][topLeftY].tileLocked) {legalPlacements[x][topLeftY] = true;
            horizontal = true;
    ...
    if (!isTouchingOld && !legalPlacements[7][7]) {
        return {
            error: 'not touching old tile ' + topLeftX + '/' + topLeftY
        };
    }
