  ...
  $.get("/game/" + this.gameKey, function (gameData, err) {
      ...
      .append(
        TABLE(
          ...
          gameData.players.map(function (player) {
            ...
            playerNumber++;
            return TR(
              { class: "player" + (playerNumber - 1) } /* ieh */,
      ...
      switch (turn.type) {
        ...
        case "swap":
          $(div).append(
            DIV(
              { class: "moveDetail" },
              "Swapped " + turn.count + " tile" + (turn.count > 1 ? "s" : "")
            )
          );
          break;
        case "challenge":
          $(div).append(
            DIV({ class: "moveDetail" }, "Challenged previous move")
          );
          break;
        case "takeBack":
          $(div).append(
            DIV({ class: "moveDetail" }, "Took back previous move")
          );
          break;
        default:
          $(div).append(
            DIV({ class: "moveDetail" }, "unknown move type " + turn.type)
          );
      }
      $("#log").append(div);
    }
      ...
      if (nextGameKey) {
        $("#log").append(
          DIV(
            { class: "nextGame" },
            A(
              { href: "/game/" + nextGameKey + "/" + $.cookie(ui.gameKey) },
              "next game"
            )
          )
        );
        ...
        $(makeNextGameButton).on("click", function () {
        ...
        $("#log").append(
          DIV(
            ...
            "Click ",
            makeNextGameButton,
            " if you want to play the same language and opponents again"
          )
        );
      }
    }
          ...
          $("#log").append(
            DIV(
              ...
              player.name +
                " gained " +
                player.tallyScore +
                " points from racks of the other players"
            )
          );
        } else {
          $("#log").append(
            DIV(
              ...
              player.name +
                " lost " +
                -player.tallyScore +
                " points for his rack containing the letters " +
                _.without(
                  player.rack.squares.map(function (square) {
                    if (square.tile) {
      ...
      $("#log").append(
        DIV(
          ...
          "Game has ended, " +
            ...
            (winners.length == 1 && !youHaveWon ? " has " : " have ") +
            "won"
        )
      );
    ...
    function placeTurnTiles(turn) {
      for (var i in turn.placements) {
        ...
        ui.board.squares[placement.x][placement.y].placeTile(
          new Tile(placement.letter, placement.score),
          true
        );
      }
    }
    ...
    $("#log").append(
      DIV({ class: "gameStart" }, gameData.language + " game started")
    );
    ...
    var lastTurn =
      gameData.turns.length && gameData.turns[gameData.turns.length - 1];
    ...
    ui.socket
      ...
      .on("turn", function (turn) {
        ...
        if (turn.type == "challenge" || turn.type == "takeBack") {
          ...
          if (turn.player == ui.playerNumber) {
            ...
            if (turn.type == "challenge") {
              ui.notify(
                "Challenged!",
                ui.players[turn.challenger].name +
                  " has challenged your move.  You have lost the " +
                  -turn.score +
                  " points you have scored and the tiles you had placed are back on your rack"
              );
            }
          }
        ...
        if (typeof turn.whosTurn == "number" && turn.type != "challenge") {
          ...
          if (turn.whosTurn == ui.playerNumber && turn.type != "takeBack") {
            ui.notify(
              "Your turn!",
              ui.players[turn.player].name +
                " has made a move and now it is your turn."
            );
            if (turn.type == "move") {
              ui.addChallengeButton();
            }
      ...
      .on("message", function (message) {
        $("#chatLog")
          .append(
            DIV(null, SPAN({ class: "name" }, message.name), ": ", message.text)
          )
  ...
  $("#dummyInput")
    ...
    .on("keydown", function (event) {
      ...
      function move(dx, dy) {
        ...
        if (ui.cursor) {
          if (dx > 0) {
            for (
              x++;
              x < ui.board.Dimension && ui.board.squares[x][y].tile;
              x++
            );
          }
          if (dx < 0) {
          ...
          if (dy > 0) {
            for (
              y++;
              y < ui.board.Dimension && ui.board.squares[x][y].tile;
              y++
            );
          }
          if (dy < 0) {
          ...
          if (
            x >= 0 &&
            x < ui.board.Dimension &&
            y >= 0 &&
            y < ui.board.Dimension &&
      ...
      function turnCursor() {
        if (ui.cursor) {
          ui.cursor.direction =
            ui.cursor.direction == "horizontal" ? "vertical" : "horizontal";
          ui.updateBoardSquare(ui.cursor.square);
        }
        handled();
      }
  ...
  if (counts.letterBag > 0) {
    $("#letterbagStatus")
      ...
      .append(
        DIV(
          null,
          SPAN({ id: "remainingTileCount" }, counts.letterBag),
          " remaining tiles"
        )
      );
    $("#scoreboard td.remainingTiles").empty();
  } else {
    $("#letterbagStatus").empty().append(DIV(null, "The letterbag is empty"));
...
UI.prototype.updateBoardSquare = function (square) {
  ...
  if (square.tile) {
        ...
        .click(function () {
          if (ui.currentlySelectedSquare) {
            ...
            function placeCursor() {
              ui.cursor = { square: square, direction: "horizontal" };
            }
            if (ui.cursor) {
    ...
    $(div).addClass("Empty").append(A(null, text));
  }
      ...
      _.range(board.Dimension).map(function (y) {
        return TR(
          ...
          _.range(board.Dimension).map(function (x) {
            ...
            return TD({ class: tdClass }, DIV({ id: id }, A()));
          })
        );
      })
    )
    ...
    TABLE(
      ...
      TR(
        ...
        _.range(8).map(function (x) {
          ...
          return TD({ class: "Normal" }, DIV({ id: id }, A()));
        })
      )
    )
  );
...
UI.prototype.selectSquare = function (square) {
  ...
  if (square && !square.tile) {
    ...
    $("#" + "Board_" + square.x + "x" + square.y).addClass("Targeted");
  }
};
  ...
  if (this.board.tileCount > 0) {
    ...
    if (move.error) {
      ...
      move.words.forEach(function (word) {
        $("#move").append(DIV(null, word.word + " " + word.score));
      });
...
UI.prototype.sendMoveToServer = function (command, args, success) {
  ...
  $.ajax({
    ...
    data: JSON.stringify({ command: command, arguments: args }),
    success: success,
    error: function (jqXHR, textStatus, errorThrown) {
      alert(
        "PUT request returned error: " + textStatus + " (" + errorThrown + ")"
      );
    },
  });
};
...
UI.prototype.swapTiles = function () {
  ...
  ui.sendMoveToServer("swap", letters, bind(this.processMoveResponse, ui));
};

UI.prototype.setMoveAction = function (action, title) {
  $("#turnButton").attr("action", action).empty().append(title);
};
...
UI.prototype.setCursor = function (square, direction) {
  if (ui.cursor) {
    ...
    ui.cursor = { square: square, direction: direction || "horizontal" };
  }
  ui.updateBoardSquare(square);
};
...
UI.prototype.TakeBackTiles = function () {
  var ui = this;
  var freeRackSquares = filter(function (square) {
    return !square.tile;
  }, ui.rack.squares);
